...
public class edgeCollisionDecision implements Decision {
	...
	public void update() {
		...
		game.level = Loyal.levels[levelWatched.newLevelIdentifier(
				entityWatchedX, entityWatchedY)];

		if (!game.level.hasBeenPlayed) {
			MapPlayer player = new MapPlayer(this.game.level, 0, 0, this.input,
					this.game);
	...
	public void addMonsters() {
		...
		for (int i = 0; i < 100; i++) {
			...
			while (tile.isSolid() == true
					|| game.level.tiles[MonsterX + MonsterY * game.level.width] == 3) {
				MonsterX = random.nextInt(399);
				MonsterY = random.nextInt(399);
				tile = game.level.getTile(MonsterX, MonsterY);
			}
			MapMoblin monster = new MapMoblin(this.game.level, MonsterX * 8,
					MonsterY * 8, 1, 64);
