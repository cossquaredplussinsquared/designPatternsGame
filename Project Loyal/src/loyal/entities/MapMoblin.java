...
public class MapMoblin extends Mob {
	...
	public boolean hasCollided(int xa, int ya) {
		...
		for (int y = yMin; y < yMax; y++) {
			if (isSolidTile(xa, ya, xMin, y) || isWater(xa, ya, xMin, y)) {
				return true;
			}
		}

		for (int y = yMin; y < yMax; y++) {
			if (isSolidTile(xa, ya, xMax, y) || isWater(xa, ya, xMax, y)) {
				return true;
			}
		}
		return false;
	...
	public void tick() {
		...
		if (Math.abs(this.x - level.getEntity(0).getX()) < stalkDistance
				&& Math.abs(this.y - level.getEntity(0).getY()) < stalkDistance) {
			stalk(stalkDistance);
	...
	public void stalk(int stalkDistance) {
		...
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < width; j++) {
				tiles[i][j] = this.level.getTile(
						(this.x - 4 - stalkDistance + (i * 8)) / 8, (this.y - 4
								- stalkDistance + (i * 8)) / 8);
			}
		}
		...
		/*
		 * if(this.x - level.getEntity(0).getX()>0 && this.y -
		 * level.getEntity(0).getY()>0) { move = 4; } else if(this.x -
		 * level.getEntity(0).getX()>0 && this.y - level.getEntity(0).getY()<0)
		 * { move = 5; } else if(this.x - level.getEntity(0).getX()<0 && this.y
		 * - level.getEntity(0).getY()>0) { move = 6; } else if(this.x -
		 * level.getEntity(0).getX()<0 && this.y - level.getEntity(0).getY()<0)
		 * { move = 7; } else if(this.x - level.getEntity(0).getX()>0) { move =
		 * 2; } else if(this.x - level.getEntity(0).getX()<0) { move =3; } else
		 * if(this.y - level.getEntity(0).getY()>0) { move = 0; } else if(this.y
		 * - level.getEntity(0).getY()<0) { move =1; }
		 */
	}
	...
	public int Pathfinder(Tile[][] tiles, int width) {
		...
		int end = (start + (level.getEntity(0).getX() - this.x) / 8 + ((level
				.getEntity(0).getY() - this.y) / 8) * width);
		int curr = start;

		for (int i = 0; i < width; i++) {
			for (int j = 0; j < width; j++) {
				AStarTiles[j + i * width] = new AStarTile(j + i * width,
						tiles[j][i].isSolid());
			}
		}
		...
		while (curr != end && curr != -1) {
			if (curr - width - 1 >= 0 && curr - width - 1 < width * width
					&& (curr - width - 1) % width != (width - 1)) {
				AStarTiles = pathScoring(curr, curr - width - 1, AStarTiles,
						14, width, end);
			}
			if (curr - width >= 0 && curr - width < width * width) {
				AStarTiles = pathScoring(curr, curr - width, AStarTiles, 10,
						width, end);
			}
			if (curr - width + 1 >= 0 && curr - width + 1 < width * width
					&& (curr - width + 1) % width != 0) {
				AStarTiles = pathScoring(curr, curr - width + 1, AStarTiles,
						14, width, end);
			}
			if (curr - 1 >= 0 && curr - 1 < width * width
					&& (curr - 1) % width != (width - 1)) {
				AStarTiles = pathScoring(curr, curr - 1, AStarTiles, 10, width,
						end);
			}
			if (curr + 1 >= 0 && curr + 1 < width * width
					&& (curr + 1) % width != 0) {
	...
	public AStarTile[] pathScoring(int curr, int tile, AStarTile[] AStarTiles,
			int distance, int width, int end) {
		if (AStarTiles[tile].isSolid() == false
				&& AStarTiles[tile].isClosed() == false) {
			if (AStarTiles[tile].isOpen() == true
					&& AStarTiles[tile].getDistance() < AStarTiles[curr]
				...
				AStarTiles[tile].setHuristic((Math.abs(end % width - tile
						% width) + Math.abs(end / width - tile / width)) * 10);
				AStarTiles[tile].setScore(distance
						+ AStarTiles[tile].getHuristic());
			}
		}
		return AStarTiles;
	}
	...
	public int lowestScore(AStarTile[] AStarTiles, int curr, int width, int end) {
		...
		if ((curr - width - 1 >= 0 && curr - width - 1 < width * width)
				&& AStarTiles[curr - width - 1].isSolid() == false
				&& AStarTiles[curr - width - 1].isClosed() == false
				&& (curr - width - 1) % width != (width - 1)) {
			next = AStarTiles[curr - width - 1];
		}
		if (curr - width >= 0 && curr - width < width * width
				&& AStarTiles[curr - width].isSolid() == false
				&& AStarTiles[curr - width].isClosed() == false
				&& AStarTiles[curr - width].getScore() < next.getScore()) {
		...
		if (curr - width + 1 >= 0 && curr - width + 1 < width * width
				...
				&& AStarTiles[curr - width + 1].isClosed() == false
				&& (curr - width + 1) % width != 0
				&& AStarTiles[curr - width + 1].getScore() < next.getScore()) {
			next = AStarTiles[curr - width + 1];
		}
		if (curr - 1 >= 0 && curr - 1 < width * width
				&& AStarTiles[curr - 1].isSolid() == false
				&& AStarTiles[curr - 1].isClosed() == false
				&& (curr - 1) % width != (width - 1)
				&& AStarTiles[curr - 1].getScore() < next.getScore()) {
